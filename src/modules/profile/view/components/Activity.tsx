    ...
    <>
      {listProjects && listProjects.length !== 0 &&
       <div className="pt-40 pb-[48px] mr-4 text-white flex flex-col gap-10">
         <p className="text-3xl md:text-[50px] font-extrabold text-white font-kode">Activity</p>
           ...
           <div className="flex flex-col lg:flex-row justify-center items-center w-full gap-4 lg:justify-between lg:items-start py-6 lg:px-20">
                   ...
                   <div className="bg-darkblack-500 p-4 border border-gray-200/20">
                     {mappedProjects?.map((project:any, index:any) => (
                       <div className="flex space-x-2" key={index}>
                        ...
                        <Image
                          unoptimized
                          src={project.icon} alt={project.symbol}
                          fill
                          sizes="100%"
                          objectFit="contain"
               ...
               >
                 <Typography className="flex items-center justify-center">
                   {mappedProjects.slice(0,10).map((item:any) => (
                     <div className="h-10 w-10 -m-[6px] border bg-darkblack-600 overflow-hidden rounded-full" key={item.symbol}>
               ...
               >
                 ...
                 <div className="flex items-center justify-center">
                   {mappedChains.map((item:any) => (
                     <div className="h-10 w-10 bg-darkblack-600 border overflow-hidden rounded-full" key={item.symbol}>
                       <img src={item.icon} alt={item.symbol} className="h-full w-full object-cover"/>
                     </div>
                   ))}
                 </div>
             ...
             <div className="flex flex-col justify-center gap-4 items-center">
               ...
               <div className="flex items-center gap-1">
                 ...
                 <div className="flex items-center justify-center">
                   <StarRating size={"text-2xl xl:text-3xl"} rating={rating ?? 0}/>
                 </div>
                 <p className="text-[#82EBFF] text-[28px]">{rating}</p>
               </div>
             </div>
             <div className="flex flex-col justify-center gap-4 items-center">
               <p className="text-[28px] text-center">Average % Per Mention</p>
