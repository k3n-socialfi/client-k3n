...
import logoColor from "@/assets/svgs/common/logo-color.svg";
import Image from "next/image";

export const CrossBtn = ({ close }: { close: () => void }) => {
  return (
...
export const HeadingLogo = () => {
  return (
    <Link href="/signin" className="block mb-7">
      <Image src={logoColor} className="block dark:hidden" alt="" />
    </Link>
  );
};
...
export const ResetPass = ({
  ...
  close: () => void;
  handelModalData: (value: string) => void;
}) => {
  return (
...
export const SuccessFull = ({ close }: { close: () => void }) => {
  return (
...
function PasswordResetModal({
  ...
  isActive: boolean;
  modalData: string;
  handleModalData: (value: string) => void;
      ...
      <div className="modal-content w-full max-w-lg mx-auto px-4">
        {modalData === "verify" ? (
          <CodeVerify
            ...
            handelModalData={handleModalData}
          />
        ) : modalData === "newPass" ? (
          <NewPass
            close={() => handleActive(false)}
