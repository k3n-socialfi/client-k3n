...
function UserData({
  ...
  return (
    <motion.tr
      ...
      className={
        index === 0
          ? "bg-darkblack-200/50 group hover:bg-darkblack-100 transition-all duration-300"
          : index === 1
          ? "bg-darkblack-300/50 group hover:bg-darkblack-100 transition-all duration-300"
          : index === 2
          ? "bg-darkblack-400/50 group hover:bg-darkblack-100 transition-all duration-300"
          : index % 2 === 0
          ? "bg-darkblack-500/50 group hover:bg-darkblack-100 transition-all duration-300"
          : "group hover:bg-darkblack-100 transition-all duration-300"
      }
    >
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col">
            <h4 className="font-bold text-lg text-white flex gap-1 items-center">
              <p className="truncate max-w-[150px]">{userInfo?.fullName}</p>
              {userInfo?.twitterInfo?.verificationStatus ? (
                <IconVerify width={20} height={19} color="#F23581" />
              ) : (
                ""
              )}
            </h4>
      ...
      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
        <span
          className={`bg-darkblack-500 hover:bg-darkblack-300 shadow-md shadow-gray-100/20 text-sm ${
            userInfo?.type === "Experts"
              ? "text-primary"
              : userInfo?.type === "KOL"
              ? "text-green-500"
              : userInfo?.type === "Celebrities"
              ? "text-secondary"
              : userInfo?.type === "Threador"
              ? "text-portage"
      ...
      <td className="whitespace-nowrap px-2 py-4 text-sm text-gray-500">
        <span
          className={`flex items-center bg-darkblack-500 hover:bg-darkblack-300 shadow-md shadow-gray-100/20 border-[1px] justify-center border-gray-100/10 px-1 py-0.5 md:px-2 md:py-1 rounded-lg text-sm ${
            userInfo.twitterInfo!.totalPoints >= 750
              ? "text-primary"
              : "text-secondary"
          } font-medium text-am `}
        >
