...
import { Top1Icon, Top2Icon, Top3Icon } from '@/assets/icons';
...
function TrendingProjects({ project}:{project: ITrendingProjects}) {
  ...
  return (
    <motion.tr
      ...
      viewport={{once: true}}
      className={
        score === 0 ? "bg-darkblack-200/50 group hover:bg-darkblack-100 transition-all duration-300 flex items-center justify-center" :
          score === 1 ? "bg-darkblack-300/50 group hover:bg-darkblack-100 transition-all duration-300 flex items-center justify-center" :
          score === 2 ? "bg-darkblack-400/50 group hover:bg-darkblack-100 transition-all duration-300 flex items-center justify-center" :
          score % 2 === 0 ? "bg-darkblack-500/50 group hover:bg-darkblack-100 transition-all duration-300 flex items-center justify-center" : "hover:bg-darkblack-100 transition-all group duration-300 flex items-center justify-center"
      }
    >
      ...
      <td className="py-4 text-sm text-gray-500 w-full">
        <div className="flex items-center gap-5">
          <Link href={`#`} className="w-[40px] h-[40px] md:w-[64px] lg:h-[64px] group-hover:w-[80px] group-hover:h-[80px] transition-all duration-300">
            <img
      ...
      <td className="whitespace-nowrap py-4 text-sm text-gray-500 pr-4">
        ...
        >
          ...
          <p className={`${getRankChangeStyle()}`}> 
            {rankChange > 0 ? `▲ ${rankChange + Math.floor(Math.random() * 31)}%` : rankChange < 0 ? `▼ ${Math.abs(rankChange) + Math.floor(Math.random() * 31)}%` : "="}
          </p>
        </Link>
      </td>
    </motion.tr>
