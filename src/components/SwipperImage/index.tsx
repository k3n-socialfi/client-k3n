        ...
        featureKols?.map((item: any, i: number) => (
          <SwiperSlide key={item?.userId}>
            <motion.div
              ...
              className="shadow-xl shadow-gray-700/30 px-2 lg:px-0"
            >
              <a
                href={`/profile/${item?.username}`}
                className="relative flex flex-col gap-6 group:relative shadow-lg text-white rounded-xl overflow-hidden bg-darkblack-500 border-[1px] border-gray-100/10 hover:bg-darkblack-200 transition-all duration-300"
              >
                <div className="absolute z-10 top-1 left-1 select-none">
                  <div className="flex items-center space-x-1 flex-wrap bg-[#191D24] border rounded-full text-white px-2 py-[1px]">
                    ...
                    <span className="text-xs">
                      {item?.twitterInfo?.totalPoints}
                    </span>{" "}
                    <span>|</span>{" "}
                    <span>
                      <IconStar width={12} height={12} />
                    </span>{" "}
                    <span className="text-xs">{item?.review}</span>
                  </div>
                </div>
                <Image
                  src={`${
                    item?.twitterInfo?.avatar ??
